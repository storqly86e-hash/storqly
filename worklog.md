...
---
Task ID: Fix npm ci + tw-animate-css build failure
Agent: Main Agent
Task: Fix Railway build failure: Can't resolve 'tw-animate-css'

Work Log:
- Railway build error: Module not found: Can't resolve 'tw-animate-css' at globals.css:3:2
- User's hypothesis: tw-animate-css not installed → partially correct
- Deeper investigation: `rm -rf node_modules && npm ci` FAILS with:
  `No matching version found for @radix-ui/react-label@^1.1.7`
- Root cause: package-lock.json was generated by bun, not npm
  - @radix-ui/react-label: package.json had ^1.1.7 (does not exist on npm)
  - bun resolved to 2.1.8 in lockfile, but npm ci rejects mismatch
  - npm ci failure → no node_modules → tw-animate-css not found → build fails
- Fixed @radix-ui/react-label to ^2.1.8 in package.json
- Deleted bun.lock (Docker uses npm, not bun)
- Regenerated package-lock.json with npm (native npm lockfile, lockfileVersion 3)
- Full clean test PASSED: rm -rf node_modules && npm ci && npm run build
  - CSS: 158,917 bytes (159KB) ✅
  - Zero errors ✅
- Pushed commit b997f20

Stage Summary:
- TWO root causes for CSS issue:
  1. tailwind.config.ts had wrong content paths (fixed in 58b181d)
  2. bun-generated lockfile incompatible with npm ci (fixed in b997f20)
